...
//  ProjectApexCore
//
//  Minimal pure-Swift SHA-256 (FIPS 180-4). No Foundation, no CryptoKit,
//  so result hashes are reproducible on any platform a future backend
//  might run on (Linux re-simulation included). Verified against the
//  standard "abc" test vector in ResultHasherTests.
//
...
public nonisolated enum SHA256 {
    ...
    public static func hexDigest(of message: [UInt8]) -> String {
        digest(of: message).map { byteToHex($0) }.joined()
    }

    /// 32-byte SHA-256 digest.
    public static func digest(of message: [UInt8]) -> [UInt8] {
        var h: [UInt32] = [
            0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
            0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19
        ]

        // Padding: append 0x80, zero-fill to 56 mod 64, append 64-bit big-endian bit length.
        var padded = message
